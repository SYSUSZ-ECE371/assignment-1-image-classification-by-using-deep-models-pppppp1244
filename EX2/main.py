...
# 添加五种数据增强方法：随机裁剪、水平翻转、垂直翻转、颜色抖动、随机旋转
# 并进行归一化和张量转换
data_transforms = transforms.Compose([
    transforms.RandomResizedCrop(224),  # 随机裁剪并缩放到224×224像素
    transforms.RandomHorizontalFlip(),  # 随机水平翻转
    transforms.RandomVerticalFlip(),    # 随机垂直翻转
    transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2),  # 颜色抖动增强
...
# Load pre-trained model and modify the last layer (using new API)
model = models.resnet18(weights=ResNet18_Weights.IMAGENET1K_V1)
# 修改最后一层全连接层，将512维特征映射到5个花卉类别
num_ftrs = model.fc.in_features
model.fc = nn.Linear(num_ftrs, 5)  # 5 classes for flower dataset
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val' and epoch_acc > best_acc:
                ...
                torch.save(best_model_wts, save_path)  # 保存模型权重
